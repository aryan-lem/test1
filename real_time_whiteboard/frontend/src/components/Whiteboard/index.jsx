...
const Whiteboard = ({socket,user,canvasRef,ctxRef,elements,setElements,tool,color}) => {
    ...
    if(!user?.presenter){
        return (
            <div className="border border-dark border-3 h-100 w-100 overflow-hidden">
                <img src={img} alt="Real time whiteboard sharing app shared by presenter" style={{height:window.innerHeight*2,width:"241%"}} />
            </div>
        )
    }
                ...
                if(element.type=="pencil"){
                    ...
                    roughCanvas.draw(
                        roughGenerator.rectangle(
                            element.offsetX,element.offsetY,element.width,element.height,{stroke:element.stroke,strokeWidth:5,roughness:0}
                        )
                    )
                }
    ...
    return ( 
        <div onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove} className="border border-dark border-3 h-100 w-100 overflow-hidden">
